...
export default function Header() {
  ...
  return <header className="site-header sticky top-0 z-50 border-b bg-card/95 backdrop-blur-xl supports-[backdrop-filter]:bg-card/85">
    <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
      <div className="flex items-center justify-between h-20 sm:h-16 gap-3">
        <button type="button" onClick={() => setLocation("/")} className="flex items-center gap-3 min-w-0 rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label="Go to Prezi Tools home">
          <img src={logoImage} alt="Prezi Tools logo" className="w-14 h-14 sm:w-12 sm:h-12 rounded-xl object-cover shadow-sm ring-1 ring-border/70" data-testid="img-logo" />
          <div className="flex items-center gap-2 min-w-0 text-left"><span className="text-lg sm:text-base font-bold tracking-tight truncate">{brand}</span>{!isBestPlays && badge && <Badge variant="secondary" className="text-[9px] sm:text-xs px-1.5 py-0 h-4 font-mono hidden xs:flex">{badge}</Badge>}{user && !billingLoading && pro && <Badge className="hidden sm:inline-flex h-5 gap-1 border border-primary/30 bg-primary/10 px-2 text-[9px] font-black text-primary hover:bg-primary/10"><Crown className="h-3 w-3" />PRO</Badge>}</div>
        </button>
        <div className="flex items-center gap-2 sm:gap-2">
          ...
          {user ? <DropdownMenu><DropdownMenuTrigger asChild><Button size="icon" variant="ghost" className="h-10 w-10 sm:h-9 sm:w-9 rounded-lg" data-testid="button-user-menu"><User className="h-5 w-5 sm:h-4 sm:w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-56"><DropdownMenuLabel className="text-xs"><div className="truncate">{user.email}</div>{!billingLoading && <div className={`mt-1 text-[9px] font-black uppercase tracking-wider ${pro ? "text-primary" : "text-muted-foreground"}`}>{pro ? "PreziTools Pro" : "Free plan"}</div>}</DropdownMenuLabel><DropdownMenuSeparator />{!billingLoading && (pro ? (manageUrl ? <DropdownMenuItem onClick={() => window.open(manageUrl, "_blank", "noopener,noreferrer")}><ExternalLink className="mr-2 h-4 w-4" />Manage Pro</DropdownMenuItem> : null) : <DropdownMenuItem onClick={startProCheckout}><Crown className="mr-2 h-4 w-4 text-primary" />Upgrade to Pro</DropdownMenuItem>)}<DropdownMenuItem onClick={handleLogout} data-testid="button-logout"><LogOut className="mr-2 h-4 w-4" />Logout</DropdownMenuItem></DropdownMenuContent></DropdownMenu> : <DropdownMenu><DropdownMenuTrigger asChild><Button size="sm" variant="ghost" className="h-10 sm:h-9 gap-2 rounded-lg px-2.5 text-sm sm:text-xs" data-testid="button-login"><LogIn className="h-5 w-5 sm:h-3.5 sm:w-3.5" /><span className="hidden xs:inline">Sign In</span></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-44"><DropdownMenuLabel className="text-xs font-normal text-muted-foreground">Optional — data is free to browse</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuItem onClick={() => setLocation("/login")}><LogIn className="mr-2 h-4 w-4" />Sign In</DropdownMenuItem><DropdownMenuItem onClick={() => setLocation("/signup")}><UserPlus className="mr-2 h-4 w-4" />Create Account</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onClick={startProCheckout}><Crown className="mr-2 h-4 w-4 text-primary" />Get Pro — $11/mo</DropdownMenuItem></DropdownMenuContent></DropdownMenu>}
          <Button size="icon" variant="ghost" className="h-10 w-10 sm:h-9 sm:w-9 rounded-lg" onClick={toggleTheme} data-testid="button-theme-toggle" aria-label={`Switch to ${theme === "light" ? "dark" : "light"} mode`}>{theme === "light" ? <Moon className="h-5 w-5 sm:h-4 sm:w-4" /> : <Sun className="h-5 w-5 sm:h-4 sm:w-4" />}</Button>
        </div>
      </div>
    </div>
  </header>;
