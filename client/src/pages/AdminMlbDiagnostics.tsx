...
export default function AdminMlbDiagnostics() {
  ...
  const query = useQuery<Diagnostics>({ queryKey: ["/api/admin/mlb/diagnostics"], queryFn: async () => { const r = await fetch("/api/admin/mlb/diagnostics", { cache: "no-store" }); if (!r.ok) throw new Error("Diagnostics unavailable"); return r.json(); }, enabled: authenticated, refetchInterval: 60_000, staleTime: 15_000 });
  ...
  return <div className="space-y-5 pb-8">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="flex items-center gap-2"><Activity className="w-5 h-5" /><h1 className="text-xl sm:text-2xl font-bold">MLB Production Diagnostics</h1></div><p className="mt-1 text-sm text-muted-foreground">Live operational checks for V4, grading, market data, verified history and CLV.</p></div><Button variant="outline" className="w-full sm:w-auto min-h-10" onClick={() => query.refetch()} disabled={query.isFetching} aria-label="Refresh MLB diagnostics"><RefreshCw className={`w-4 h-4 mr-2 ${query.isFetching ? "animate-spin" : ""}`} />Refresh</Button></div>
