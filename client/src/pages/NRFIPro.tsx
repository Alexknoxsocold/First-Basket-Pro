...
type Filter = "plays" | "nrfi" | "yrfi" | "outcomes";
...
export default function NRFIPro() {
  ...
  const nrfiGames = [...games.filter(g => g.recommendation === "NRFI")].sort((a, b) => b.modelEdge - a.modelEdge);
  const yrfiGames = [...games.filter(g => g.recommendation === "YRFI")].sort((a, b) => b.modelEdge - a.modelEdge);
  ...
  return (
    <div className="-mx-4 md:-mx-6 lg:-mx-8 -mt-8">
      <nav className="border-b bg-card"><div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8"><div className="flex items-center gap-0 overflow-x-auto">
        {(["plays", "nrfi", "yrfi", "outcomes"] as Filter[]).map(tab => {
          const count = tab === "plays" ? promoted.length : tab === "nrfi" ? nrfiGames.length : tab === "yrfi" ? yrfiGames.length : games.filter(g => g.outcome === "won").length;
          const label = tab === "plays" ? "Value Plays" : tab === "nrfi" ? "NRFI" : tab === "yrfi" ? "YRFI" : "Results";
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-8">
        <div className="flex flex-wrap items-start justify-between gap-3 mb-6"><div><h1 className="text-lg font-bold">MLB First-Inning Value</h1><p className="text-xs text-muted-foreground mt-1">{data ? formatDate(data.date) : "Today"} · NRFI and YRFI show every model prediction; Value Plays only promotes stronger signals</p></div><Button variant="outline" size="sm" onClick={() => refetch()} disabled={isFetching} className="gap-2 text-xs"><RefreshCw className={`w-3.5 h-3.5 ${isFetching ? "animate-spin" : ""}`} />Refresh</Button></div>
        {error ? <div className="rounded-md border border-destructive/30 bg-destructive/5 p-5 text-sm text-destructive">MLB data could not be loaded right now. ESPN may be temporarily unavailable.</div> : <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            ...
            <div className="rounded-md border bg-card p-5"><div className="flex items-center justify-between mb-3"><span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Top NRFI Model</span><TrendingUp className="w-5 h-5 text-primary" /></div><div className="text-3xl font-bold">{topNrfi ? `${topNrfi.nrfiProbability.toFixed(1)}%` : "—"}</div><div className="text-xs text-muted-foreground mt-1">{topNrfi?.shortName ?? "No NRFI games"}{topNrfi && topNrfi.playStatus === "NO_PLAY" ? " · pass" : ""}</div></div>
            <div className="rounded-md border bg-card p-5"><div className="flex items-center justify-between mb-3"><span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Top YRFI Model</span><TrendingUp className="w-5 h-5 text-primary" /></div><div className="text-3xl font-bold">{topYrfi ? `${(100 - topYrfi.nrfiProbability).toFixed(1)}%` : "—"}</div><div className="text-xs text-muted-foreground mt-1">{topYrfi?.shortName ?? "No YRFI games"}{topYrfi && topYrfi.playStatus === "NO_PLAY" ? " · pass" : ""}</div></div>
          </div>

          {filter === "plays" && <div className="rounded-md border bg-primary/5 p-4 mb-6"><div className="flex items-center gap-2 text-sm font-semibold"><Zap className="w-4 h-4 text-primary" />What “Value Plays” means</div><p className="text-xs text-muted-foreground mt-1">A Value Play is not simply the highest probability. It means the model is meaningfully separated from the 50% neutral baseline and has enough recent data to justify promotion. This endpoint does not yet ingest live sportsbook odds, so these are model-strength plays — not guaranteed market edge.</p><div className="flex items-start gap-2 mt-3 text-xs text-muted-foreground"><Info className="w-3.5 h-3.5 mt-0.5 shrink-0" /><span>True betting value comes from comparing our probability with the current no-vig market probability. Until live odds are connected, the site intentionally prefers fewer, higher-confidence recommendations.</span></div></div>}

          <div className="flex items-start justify-between gap-3 mb-4"><div><h2 className="text-base font-bold">{filter === "plays" ? "Top Value Plays" : filter === "outcomes" ? "Winning Outcomes" : `${filter.toUpperCase()} Predictions`}</h2><p className="text-xs text-muted-foreground mt-1">{filter === "plays" ? "Only meaningful model edges are promoted here." : filter === "nrfi" ? "Every NRFI prediction is shown here, including passes around 50%." : filter === "yrfi" ? "Every YRFI prediction is shown here, including passes around 50%." : "Historical results from completed predictions."}</p></div><span className="text-[10px] text-muted-foreground">Updated {data ? formatTime(data.updatedAt) : "—"}</span></div>
