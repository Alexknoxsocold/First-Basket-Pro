...
function teamLogo(team:MlbTeam|undefined){return typeof team==='object'&&team?team.logo??null:null}
function PlayAvatar({p,index}:{p:Play;index:number}){
  ...
  if(p.awayLogo||p.homeLogo)return <div className="relative w-12 h-11 shrink-0"><div className="absolute left-0 top-1 w-9 h-9 rounded-full bg-background border shadow-sm flex items-center justify-center overflow-hidden">{p.awayLogo?<img src={p.awayLogo} alt={`${p.awayAbbr||'Away'} logo`} className="w-7 h-7 object-contain"/>:<span className="text-[8px] font-bold">{p.awayAbbr}</span>}</div><div className="absolute right-0 bottom-0 w-9 h-9 rounded-full bg-background border shadow-sm flex items-center justify-center overflow-hidden">{p.homeLogo?<img src={p.homeLogo} alt={`${p.homeAbbr||'Home'} logo`} className="w-7 h-7 object-contain"/>:<span className="text-[8px] font-bold">{p.homeAbbr}</span>}</div></div>;
...
export default function BestPlays(){
 ...
 const plays=useMemo(()=>{
   ...
   for(const g of mg){
     ...
     const awayAbbr=teamName(g.away),homeAbbr=teamName(g.home);
     const id=String(g.gamePk??g.id??`${awayAbbr}-${homeAbbr}-${g.date??g.gameTime??''}`);
