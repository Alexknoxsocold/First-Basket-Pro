...
export default function MLBHomeRuns() {
  ...
  return <div className="space-y-5 pb-3">
    ...
    {error ? <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-5 text-sm text-destructive">MLB home-run data could not be loaded right now.</div> : <>
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><div className="rounded-xl border bg-card/80 p-4"><div className="text-[9px] uppercase tracking-[.12em] text-muted-foreground">Strong HR value</div><div className="mt-1 font-mono text-2xl font-black">{valueRows.length}</div><div className="text-[9px] text-muted-foreground">Pregame, verified, non-lottery prices</div></div><div className="rounded-xl border bg-card/80 p-4"><div className="text-[9px] uppercase tracking-[.12em] text-muted-foreground">Most likely HR</div><div className="mt-1 font-mono text-2xl font-black">{top ? `${top.probability.toFixed(1)}%` : '—'}</div><div className="truncate text-[9px] text-muted-foreground">{top?.player ?? 'Waiting for data'}</div></div><div className="rounded-xl border bg-card/80 p-4"><div className="text-[9px] uppercase tracking-[.12em] text-muted-foreground">Players priced</div><div className="mt-1 font-mono text-2xl font-black">{data?.marketPlayersPriced ?? 0}</div><div className="text-[9px] text-muted-foreground">Live PropLine HR market</div></div><div className="rounded-xl border bg-card/80 p-4"><div className="text-[9px] uppercase tracking-[.12em] text-muted-foreground">Ranked hitters</div><div className="mt-1 font-mono text-2xl font-black">{confirmedRows.length}</div><div className="text-[9px] text-muted-foreground">Top confirmed pregame HR probabilities</div></div></div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.12fr_.88fr]"><section className="min-w-0"><div className="mb-2"><div className="flex items-center gap-2"><h2 className="text-sm font-bold">Most Likely HR</h2><Badge variant="outline" className="h-5 border-emerald-500/25 bg-emerald-500/[.08] px-1.5 text-[9px] text-emerald-600">{confirmedRows.length}</Badge></div><p className="mt-0.5 text-[10px] text-muted-foreground">Top 15 confirmed hitters by HR probability. POWER PLAY and STRONG labels remain selective; WATCH means ranked context, not a recommended bet.</p></div><div className="max-h-[760px] overflow-y-auto rounded-2xl border border-emerald-500/15 bg-card/80">{confirmedRows.length ? confirmedRows.map(row => <ConfirmedRow key={`${row.gamePk}-${row.playerId}`} row={row} onOpen={setSelectedPlayer} />) : <div className="px-5 py-12 text-center"><Clock3 className="mx-auto h-7 w-7 text-muted-foreground/35" /><div className="mt-2 text-sm font-semibold">No upcoming confirmed HR hitters</div></div>}</div></section><section className="min-w-0"><div className="mb-2"><h2 className="text-sm font-bold">Early HR Watchlist</h2><p className="mt-0.5 text-[10px] text-muted-foreground">Power profiles before official batting orders post.</p></div><div className="max-h-[500px] overflow-y-auto rounded-xl border bg-card/75">{watchRows.length ? watchRows.map(row => <WatchRow key={`${row.gamePk}-${row.playerId}`} row={row} onOpen={setSelectedPlayer} />) : <div className="px-5 py-12 text-center"><Activity className="mx-auto h-7 w-7 text-muted-foreground/35" /><div className="mt-2 text-sm font-semibold">No early watchlist candidates</div></div>}</div></section></div>
