...
function ConfirmedRow({ row, onOpen }: { row: Candidate; onOpen: (row: Candidate) => void }) {
  ...
  return <button type="button" onClick={() => onOpen(row)} className="grid w-full grid-cols-[auto_minmax(0,1fr)_125px] items-center gap-3 border-b border-border/45 px-4 py-3.5 text-left transition-all last:border-b-0 hover:bg-emerald-500/[.045] focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/40">
    ...
    <div className="text-right"><div className="font-mono text-2xl font-black">{row.probability.toFixed(1)}%</div><div className="text-[8px] uppercase tracking-[.14em] text-muted-foreground">HR probability</div>{best && <div className="mt-2 flex justify-end"><div className="flex items-center gap-1.5"><SportsbookLogo quote={best} /><span className="font-mono text-[10px] font-bold">{odds(best.americanOdds)}</span></div></div>}</div>
  </button>;
}

function ValueRow({ row, onOpen }: { row: Candidate; onOpen: (row: Candidate) => void }) {
  const m = row.market!;
  const books = m.quotes.slice(0, 3);
  return <button type="button" onClick={() => onOpen(row)} className="grid w-full grid-cols-[auto_minmax(0,1fr)] gap-3 border-b border-border/45 px-4 py-3.5 text-left transition-all last:border-b-0 hover:bg-violet-500/[.045] focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/40 md:grid-cols-[auto_minmax(0,1fr)_250px] md:items-center">
    <PlayerHeadshot row={row} />
    <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="truncate text-[15px] font-bold">{row.player}</span><Badge variant="outline" className="border-violet-500/40 bg-violet-500/10 text-violet-600 dark:text-violet-300">STRONG VALUE</Badge></div><div className="mt-0.5 text-[10px] text-muted-foreground">{row.team} vs {row.opponent} · {time(row.gameTime)}</div><div className="mt-1.5 text-[9px] text-muted-foreground">Model {row.probability.toFixed(1)}% · Market {m.consensusImpliedProbability.toFixed(1)}% · {m.trustedQuoteCount} verified books</div><div className="mt-1 font-mono text-[10px] font-bold text-emerald-600">+{m.modelEdge.toFixed(1)}% model edge</div></div>
    <div className="col-span-2 grid grid-cols-1 gap-1.5 sm:grid-cols-3 md:col-span-1">{books.map((q, i) => <BookPrice key={`${q.bookmakerKey}-${i}`} quote={q} compact />)}</div>
  </button>;
}
...
function PlayerDetailModal({ row, onClose }: { row: Candidate; onClose: () => void }) {
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/65 p-4 backdrop-blur-sm" onMouseDown={e => { if (e.currentTarget === e.target) onClose(); }}><div role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border bg-card shadow-2xl">
    ...
    <div className="space-y-5 p-5">
      ...
      {row.market && <div className="rounded-xl border border-violet-500/20 bg-violet-500/[.06] p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div className="text-sm font-bold">Live sportsbook prices</div><Badge variant="outline" className={row.market.priceVerified ? 'border-emerald-500/30 bg-emerald-500/[.08] text-emerald-600' : 'border-yellow-500/30 bg-yellow-500/[.08] text-yellow-600'}>{row.market.priceVerified ? `${row.market.trustedQuoteCount} trustworthy books` : 'price not verified'}</Badge></div><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3"><DetailMetric label="Best Trusted Price" value={odds(row.market.bestOdds)} sub={row.market.bestBook} /><DetailMetric label="Market Consensus" value={`${row.market.consensusImpliedProbability.toFixed(1)}%`} sub={`${row.market.quoteCount} books received`} /><DetailMetric label="Model Edge" value={`${row.market.modelEdge >= 0 ? '+' : ''}${row.market.modelEdge.toFixed(1)}%`} /></div><div className="mt-4 grid gap-2 sm:grid-cols-2">{row.market.quotes.map((q, i) => <BookPrice key={`${q.bookmakerKey}-${i}`} quote={q} />)}</div>{row.market.outlierQuoteCount > 0 && <div className="mt-3 rounded-md border border-yellow-500/20 bg-yellow-500/[.06] px-3 py-2 text-[9px] text-muted-foreground">{row.market.outlierQuoteCount} extreme quote{row.market.outlierQuoteCount === 1 ? '' : 's'} detected. They remain visible for transparency but do not create a main value play.</div>}</div>}
...
export default function MLBHomeRuns() {
  ...
  return <div className="space-y-5 pb-3">
    ...
    {error ? <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-5 text-sm text-destructive">MLB home-run data could not be loaded right now.</div> : <>
      ...
      <div className={`rounded-xl border px-4 py-3 ${data?.marketStatus === 'available' ? 'border-violet-500/20 bg-violet-500/[.06]' : 'border-yellow-500/20 bg-yellow-500/[.06]'}`}><div className="flex items-center gap-2 text-sm font-semibold"><ShieldCheck className="h-4 w-4" />{data?.marketStatus === 'available' ? 'Sportsbook prices are live. Main value now favors realistic pregame prices and stronger HR probability instead of raw EV.' : data?.note}</div></div>

      <section><div className="mb-2"><div className="flex items-center gap-2"><h2 className="text-sm font-bold">Strong HR Value</h2><Badge variant="outline" className="border-violet-500/30 bg-violet-500/10 text-violet-600">{valueRows.length}</Badge></div><p className="mt-0.5 text-[10px] text-muted-foreground">Only upcoming games, confirmed lineups, verified multi-book prices, at least 12% model HR probability and prices no longer than +1200. Raw EV is kept out of the public ranking.</p></div><div className="max-h-[520px] overflow-y-auto rounded-2xl border border-violet-500/15 bg-card/80">{valueRows.length ? valueRows.map(row => <ValueRow key={`value-${row.gamePk}-${row.playerId}`} row={row} onOpen={setSelectedPlayer} />) : <div className="px-5 py-10 text-center"><Crosshair className="mx-auto h-7 w-7 text-muted-foreground/35" /><div className="mt-2 text-sm font-semibold">No strong HR value right now</div><div className="mt-1 text-[10px] text-muted-foreground">That is a valid result. The page will not fill itself with long-shot tickets just because the math says positive EV.</div></div>}</div></section>
      ...
      <div className="rounded-md border bg-card/60 p-3 text-[9px] leading-relaxed text-muted-foreground"><span className="font-semibold text-foreground">Model {data?.modelVersion ?? 'hr-v2-value'}:</span> {data?.methodology} Public rankings suppress already-started games and extreme long-shot value. Probabilities are model estimates, not guarantees.</div>
    </>}
  </div>;
}
