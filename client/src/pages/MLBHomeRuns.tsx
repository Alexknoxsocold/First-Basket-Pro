...
function quoteSummary(market: HrMarket, limit = 3) { return market.quotes.slice(0, limit).map(q => `${q.bookmaker} ${odds(q.americanOdds)}`).join(' · '); }
...
function ConfirmedRow({ row, onOpen }: { row: Candidate; onOpen: (row: Candidate) => void }) {
  return <button type="button" onClick={() => onOpen(row)} className="grid w-full grid-cols-[auto_minmax(0,1fr)_110px] items-center gap-3 border-b border-border/45 px-4 py-3.5 text-left transition-all last:border-b-0 hover:bg-emerald-500/[.045] focus:outline-none focus-visible:bg-muted/40 focus-visible:ring-2 focus-visible:ring-primary/40">
    ...
    <div className="text-right"><div className="font-mono text-2xl font-black tracking-tight">{row.probability.toFixed(1)}%</div><div className="text-[8px] uppercase tracking-[.14em] text-muted-foreground">HR probability</div>{row.market && <div className="mt-1 text-[9px] text-muted-foreground">{row.market.bestBook} {odds(row.market.bestOdds)}</div>}</div>
  </button>;
}
...
function ValueRow({ row, onOpen }: { row: Candidate; onOpen: (row: Candidate) => void }) {
  ...
  return <button type="button" onClick={() => onOpen(row)} className="grid w-full grid-cols-[auto_minmax(0,1fr)_135px] items-center gap-3 border-b border-border/45 px-4 py-3.5 text-left transition-all last:border-b-0 hover:bg-violet-500/[.045] focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500/40">
    <PlayerHeadshot row={row} size="lg" />
    <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="truncate text-[15px] font-bold">{row.player}</span><Badge variant="outline" className={m.valueTier === 'BEST_VALUE' ? 'border-violet-500/40 bg-violet-500/10 text-violet-600 dark:text-violet-300' : 'border-cyan-500/35 bg-cyan-500/10 text-cyan-600 dark:text-cyan-300'}>{m.valueTier === 'BEST_VALUE' ? 'BEST VALUE' : 'VALUE'}</Badge><Badge variant="outline" className="border-emerald-500/30 bg-emerald-500/[.08] text-[8px] text-emerald-600">{m.trustedQuoteCount} BOOKS VERIFIED</Badge></div><div className="mt-0.5 text-[10px] text-muted-foreground">{row.team} vs {row.opponent} · {time(row.gameTime)}</div><div className="mt-1.5 text-[9px] text-muted-foreground">Model {row.probability.toFixed(1)}% · Market consensus {m.consensusImpliedProbability.toFixed(1)}%</div><div className="mt-1 truncate text-[9px] text-muted-foreground">{quoteSummary(m, 4)}</div></div>
    <div className="text-right"><div className="font-mono text-2xl font-black">{odds(m.bestOdds)}</div><div className="text-[8px] uppercase tracking-[.14em] text-muted-foreground">{m.bestBook} best</div><div className="mt-1 font-mono text-[11px] font-bold text-emerald-600">+{m.modelEdge.toFixed(1)}% edge</div><div className="text-[9px] text-muted-foreground">EV {m.expectedValue >= 0 ? '+' : ''}{m.expectedValue.toFixed(1)}%</div></div>
  </button>;
}
...
function PlayerDetailModal({ row, onClose }: { row: Candidate; onClose: () => void }) {
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/65 p-4 backdrop-blur-sm" onMouseDown={e => { if (e.currentTarget === e.target) onClose(); }}><div role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border bg-card shadow-2xl">
    ...
    <div className="space-y-5 p-5">
      ...
      {row.market && <div className="rounded-xl border border-violet-500/20 bg-violet-500/[.06] p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div className="text-sm font-bold">Live HR sportsbook market · PropLine</div><Badge variant="outline" className={row.market.priceVerified ? 'border-emerald-500/30 bg-emerald-500/[.08] text-emerald-600' : 'border-yellow-500/30 bg-yellow-500/[.08] text-yellow-600'}>{row.market.priceVerified ? `${row.market.trustedQuoteCount} trustworthy books` : 'price not verified'}</Badge></div><div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4"><DetailMetric label="Best Trusted Price" value={odds(row.market.bestOdds)} sub={row.market.bestBook} /><DetailMetric label="Market Consensus" value={`${row.market.consensusImpliedProbability.toFixed(1)}%`} sub={`${row.market.quoteCount} books received`} /><DetailMetric label="Model Edge" value={`${row.market.modelEdge >= 0 ? '+' : ''}${row.market.modelEdge.toFixed(1)}%`} /><DetailMetric label="Expected Value" value={`${row.market.expectedValue >= 0 ? '+' : ''}${row.market.expectedValue.toFixed(1)}%`} /></div><div className="mt-4 text-[9px] font-semibold uppercase tracking-[.12em] text-muted-foreground">Sportsbook prices received</div><div className="mt-2 grid gap-2 sm:grid-cols-2">{row.market.quotes.map((q, index) => <div key={`${q.bookmakerKey}-${index}`} className="flex items-center justify-between rounded-lg border bg-card/55 px-3 py-2 text-[10px]"><span className="font-medium">{q.bookmaker}</span><span className="font-mono font-bold">{odds(q.americanOdds)}</span></div>)}</div>{row.market.outlierQuoteCount > 0 && <div className="mt-3 rounded-md border border-yellow-500/20 bg-yellow-500/[.06] px-3 py-2 text-[9px] text-muted-foreground">{row.market.outlierQuoteCount} extreme quote{row.market.outlierQuoteCount === 1 ? '' : 's'} detected. Those prices remain visible here but are excluded from value qualification.</div>}</div>}
...
export default function MLBHomeRuns() {
  ...
  return <div className="space-y-5 pb-3">
    ...
    <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><Flame className="h-5 w-5 text-orange-500" /><h1 className="text-xl font-bold">Homeruns</h1></div><p className="mt-1 max-w-3xl text-xs text-muted-foreground">Independent HR probability plus live multi-sportsbook value from PropLine. Most likely and best value are ranked separately.</p></div>{updatedTime(data?.updatedAt) && <div className="text-right text-[9px] text-muted-foreground">Last updated: {updatedTime(data?.updatedAt)}</div>}</div>
    ...
    {error ? <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-5 text-sm text-destructive">MLB home-run data could not be loaded right now.</div> : <>
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><div className="rounded-xl border bg-card/80 p-4"><div className="text-[9px] uppercase tracking-[.12em] text-muted-foreground">Best HR value</div><div className="mt-1 font-mono text-2xl font-black">{valueRows.length}</div><div className="text-[9px] text-muted-foreground">Verified multi-book edge + EV</div></div><div className="rounded-xl border bg-card/80 p-4"><div className="text-[9px] uppercase tracking-[.12em] text-muted-foreground">Most likely HR</div><div className="mt-1 font-mono text-2xl font-black">{top ? `${top.probability.toFixed(1)}%` : '—'}</div><div className="truncate text-[9px] text-muted-foreground">{top?.player ?? 'Waiting for data'}</div></div><div className="rounded-xl border bg-card/80 p-4"><div className="text-[9px] uppercase tracking-[.12em] text-muted-foreground">Players priced</div><div className="mt-1 font-mono text-2xl font-black">{data?.marketPlayersPriced ?? 0}</div><div className="text-[9px] text-muted-foreground">PropLine batter_home_runs market</div></div><div className="rounded-xl border bg-card/80 p-4"><div className="text-[9px] uppercase tracking-[.12em] text-muted-foreground">Confirmed plays</div><div className="mt-1 font-mono text-2xl font-black">{confirmedRows.length}</div><div className="text-[9px] text-muted-foreground">Probability-first rankings</div></div></div>
