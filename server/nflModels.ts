...
async function teamProfile(team:string){
  ...
  const urls=[
    ...
    `${ESPN_SITE}/teams/${encodeURIComponent(team)}/statistics`,
  ];
  for(const url of urls){try{const payload=await getJson<unknown>(url);const m=new Map<string,number>();collectStats(payload,m);const ppg=stat(m,'pointsPerGame','avgPoints','points');const papg=stat(m,'pointsAllowedPerGame','avgPointsAllowed','pointsAgainst');const ypg=stat(m,'yardsPerGame','totalYardsPerGame');const takeaways=stat(m,'takeaways');const giveaways=stat(m,'giveaways','turnovers');if(ppg!==null||ypg!==null)return{ppg,papg,ypg,takeaways,giveaways};}catch{} }
  return{ppg:null,papg:null,ypg:null,takeaways:null,giveaways:null};
}
...
export async function modelMoneyline(game:ModelGameInput,awayOdds:number|null,homeOdds:number|null){
  ...
  let rating=(homeRecord-awayRecord)*2.4+0.12; // modest home-field baseline
  ...
  const build=(prob:number,odds:number|null):ModelResult|null=>{
    ...
    const implied=odds>0?100/(odds+100):Math.abs(odds)/(Math.abs(odds)+100)*100;
    const edge=prob-implied;const expected=ev(prob,odds);
...
async function roster(team:string){
  ...
  try{
    const payload=await getJson<any>(`${ESPN_SITE}/teams/${encodeURIComponent(team)}/roster`);
    const players:{id:string;name:string;position:string}[]=[];
    const walk=(node:any)=>{if(!node)return;if(Array.isArray(node)){node.forEach(walk);return;}if(typeof node!=='object')return;const id=node.id??node.athlete?.id;const name=node.fullName??node.displayName??node.athlete?.fullName??node.athlete?.displayName;const position=node.position?.abbreviation??node.athlete?.position?.abbreviation;if(id&&name)players.push({id:String(id),name:String(name),position:String(position??'')});Object.values(node).forEach(walk);};walk(payload);
    const unique=[...new Map(players.map(p=>[p.id,p])).values()];rosterCache.set(key,{expires:Date.now()+MODEL_CACHE_MS,players:unique});return unique;
  }catch{return[];}
}

async function playerBaseline(playerName:string,teams:string[]){
  for(const team of teams){const players=await roster(team);const target=norm(playerName);const found=players.find(p=>norm(p.name)===target)||players.find(p=>norm(p.name).endsWith(target)||target.endsWith(norm(p.name)));if(!found)continue;
    const season=new Date().getUTCFullYear()-1;
    try{const payload=await getJson<unknown>(`${ESPN_WEB}/athletes/${encodeURIComponent(found.id)}/stats?season=${season}&seasontype=2`);const m=new Map<string,number>();collectStats(payload,m);const rush=stat(m,'rushingTouchdowns','rushTouchdowns')??0;const rec=stat(m,'receivingTouchdowns','recTouchdowns')??0;const games=stat(m,'gamesPlayed','games')??17;const touches=(stat(m,'rushingAttempts','carries')??0)+(stat(m,'receptions')??0);const td=rush+rec;return{position:found.position,td,games:Math.max(1,games),touches};}catch{return{position:found.position,td:0,games:17,touches:0};}
  }
  return null;
}
...
export async function qualifyTdMarkets(game:ModelGameInput,rows:ModelPlayerMarket[],kind:'anytime'|'first'){
  ...
  for(const row of rows){
    const base=await playerBaseline(row.player,teams);if(!base)continue;
    const tdPerGame=base.td/base.games;
    const usageBoost=clamp(base.touches/base.games/20,0,0.18);
    let probability=kind==='anytime'?(1-Math.exp(-(tdPerGame+usageBoost)))*100:(1-Math.exp(-(tdPerGame+usageBoost)))*22;
    if(['RB','WR','TE'].includes(base.position))probability*=1.05;
    probability=clamp(probability,kind==='anytime'?2:0.8,kind==='anytime'?72:24);
    const edge=probability-row.impliedProbability;const expected=ev(probability,row.bestOdds);
    const minBooks=kind==='first'?2:1;
    const minEdge=kind==='first'?3.5:2.5;
    const minEv=kind==='first'?0.08:0.05;
    const confidence:ModelConfidence=edge>=7&&expected>=0.12?'elite':edge>=minEdge&&expected>=minEv?'strong':'watch';
    const qualifies=row.quoteCount>=minBooks&&edge>=minEdge&&expected>=minEv&&(kind==='first'?probability>=5:probability>=18);
    if(qualifies)output.push({...row,modelProbability:+probability.toFixed(1),edgePoints:+edge.toFixed(1),expectedValue:+expected.toFixed(3),confidence,qualifies:true,reasons:[`${base.td} TD in ${base.games} games last season`,`${base.position||'skill'} role`,`${base.touches} season touches`,`${row.quoteCount} sportsbook quote${row.quoteCount===1?'':'s'}`]});
  }
