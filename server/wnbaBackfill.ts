...
export async function refreshRecentWnbaEvidence(days=10):Promise<{checked:number;updated:number;rejected:number}>{
  ...
  for(const r of rows.rows){checked++;const summary=await json(`https://site.api.espn.com/apis/site/v2/sports/basketball/wnba/summary?event=${r.espn_game_id}`),starters=extractStarters(summary),e=verifyOpeningEvidence(String(r.espn_game_id),String(r.game_date).slice(0,10),summary,starters);if(!e){rejected++;continue}await saveOpeningEvidence(e);updated++}
