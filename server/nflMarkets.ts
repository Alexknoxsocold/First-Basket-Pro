...
import { modelMoneylineV2, type NflMoneylineV2Result } from './nflMoneylineV2.js';
...
export type NflMoneylineSide = { team:string; bestOdds:number|null; bestBook:string|null; impliedProbability:number|null; consensusNoVigProbability:number|null; quotes:NflBookQuote[]; model?:NflModelFields|null; v2Shadow?:NflMoneylineV2Result|null; };
...
export async function fetchNflMarkets():Promise<NflMarketFeed>{
  ...
  for(const game of games){
    ...
    if(rawMoneyline){
      try{
        // V1 remains the production gate.
        ...
        // V2 runs beside V1 only. It cannot create/remove a production pick.
        try{
          ...
          rawMoneyline.away.v2Shadow=v2.away;rawMoneyline.home.v2Shadow=v2.home;
        }catch(error){console.warn(`[NFL Model V2 Shadow] failed for ${game.id}:`,error);}
