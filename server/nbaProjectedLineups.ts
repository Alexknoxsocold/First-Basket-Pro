...
function extractRosterCandidates(data: any, team: TeamAbbr): Candidate[] {
  ...
  return athletes.map((entry: any, index: number) => ({
    id: athleteId(entry),
    player: athleteName(entry),
    team,
    position: positionFrom(entry),
    headshot: athleteHeadshot(entry),
    injuryStatus: athleteInjury(entry),
    depthRank: index + 1,
  })).filter((player: Candidate) => player.id && player.player && !unavailable(player.injuryStatus));
}
...
async function projectTeam(team: TeamAbbr, previousEspnSeason: number): Promise<Array<Candidate & PriorStats & { projectionSource: ProjectedNbaPlayer['projectionSource'] }>> {
  const [depthData, rosterData] = await Promise.all([
    fetchJson(`https://site.api.espn.com/apis/site/v2/sports/basketball/nba/teams/${team}/depthcharts`),
    fetchJson(`https://site.api.espn.com/apis/site/v2/sports/basketball/nba/teams/${team}/roster`),
  ]);
...
export async function getNbaProjectedLineups(forceRefresh = false): Promise<CachedProjection> {
  ...
  projectionCache = { value, expiresAt: Date.now() + CACHE_TTL_MS };
  return value;
}
