...
function isFullModelSlate(slate: WnbaSlate | null | undefined): slate is WnbaSlate {
  if (!slate || !Array.isArray(slate.games) || !slate.games.length) return false;
  if (String(slate.source || '').includes('prediction ledger fallback')) return false;
  const candidates = slate.games.flatMap(game => game.candidates || []);
  if (!candidates.length) return false;
  return candidates.some(player => Boolean(player.headshot) || player.avgMinutes > 0 || player.avgPoints > 0 || player.seasonStarts > 0);
}

async function saveSnapshot(slate: WnbaSlate): Promise<void> {
  if (!pool || !isFullModelSlate(slate)) return;
...
async function loadSnapshot(): Promise<WnbaSlate | null> {
  ...
  if (!isFullModelSlate(payload)) return null;
  return {
    ...payload,
...
export async function getResilientWnbaSlate(force = false): Promise<WnbaSlate> {
  try {
    ...
    if (cached) {
      console.warn('[WNBA Fail-safe] Live feed was incomplete; serving persisted full-model slate instead.');
      return cached;
    }
    return live;
  } catch (error) {
    ...
    if (cached) {
      console.warn('[WNBA Fail-safe] Live slate failed; serving persisted full-model slate instead.', error);
      return cached;
    }
    throw error;
  }
