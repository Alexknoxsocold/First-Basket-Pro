...
async function fetchTeamForm(teamId: string, beforeDate: string, sharedHistory: EspnEvent[]): Promise<TeamForm> { const cacheKey = `${teamId}:${beforeDate}`; const cached = teamFormCache.get(cacheKey); if (cached && cached.expiresAt > Date.now()) return cached.value; const value = calculateTeamForm(teamId, sharedHistory); teamFormCache.set(cacheKey, { value, expiresAt: HISTORY_CACHE_TTL }); return value; }
