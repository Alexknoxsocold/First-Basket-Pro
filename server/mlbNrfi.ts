...
/**
 * One scoreboard request contains every MLB game for a date, including linescores.
 * Using daily scoreboards avoids the old N-teams x N-games summary-request explosion.
 */
...
async function fetchTeamForm(teamId: string, beforeDate: string): Promise<TeamForm> {
  ...
  try {
    const allRecentGames = await fetchRecentHistory(beforeDate);
...
function pitcherRunAdjustment(pitcher: NrfiPitcher): number {
  return pitcher.era === null ? 1 : clamp(pitcher.era / 4.25, 0.80, 1.25);
}
...
function buildPrediction(
  ...
  const confidence = sampleSize >= 8 && pitcherKnown
    ? "High"
    : sampleSize >= 5 && (pitcherKnown || awayPitcher.name !== null || homePitcher.name !== null)
      ? "Medium"
      : "Low";
  ...
  if (awayForm.noRunPct >= 0.55 && homeForm.noRunPct >= 0.55) {
    ...
    factors.push("Recent first-inning results are near the league baseline");
  }
  if (pitcherKnown) {
...
async function buildNrfiData(date: string): Promise<NrfiResponse> {
  ...
  // All team histories are built from the same small set of daily scoreboard calls.
  // Fetching them concurrently prevents the first request from serially loading each team.
  ...
  const forms = await withConcurrency(teamIds, async teamId => [teamId, await fetchTeamForm(teamId, date)] as const, 12);
  ...
  return {
    ...
    source: "ESPN MLB scoreboards + verified recent first-inning linescores",
