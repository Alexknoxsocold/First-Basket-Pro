...
// Pregame data is intentionally refreshed frequently so a scratched/replaced
// probable starter can change the prediction without waiting 20 minutes.
...
async function fetchMlbPitcherStats(name: string): Promise<{ era: number | null; whip: number | null }> {
  ...
  if (!key) return { era: null, whip: null };
  const cached = pitcherCache.get(key);
  if (cached && cached.expiresAt > Date.now()) return cached.value;

  const empty = { era: null, whip: null };
  try {
    const search = await fetchJson<{ people?: { id?: number; fullName?: string }[] }>(
      `${MLB_BASE}/people/search?names=${encodeURIComponent(name)}&active=true&sportIds=1`,
      5000,
    );
    ...
    if (!match?.id) {
      pitcherCache.set(key, { value: empty, expiresAt: Date.now() + PITCHER_CACHE_TTL });
      return empty;
    }

    const season = new Date().getUTCFullYear();
    const stats = await fetchJson<{ stats?: { splits?: { stat?: { era?: string | number; whip?: string | number } }[] }[] }>(
      `${MLB_BASE}/people/${match.id}/stats?stats=season&group=pitching&season=${season}`,
      5000,
    );
    const stat = stats.stats?.[0]?.splits?.[0]?.stat;
    const era = stat?.era !== undefined ? Number(stat.era) : null;
    const whip = stat?.whip !== undefined ? Number(stat.whip) : null;
    const value = {
      era: Number.isFinite(era as number) ? era : null,
      whip: Number.isFinite(whip as number) ? whip : null,
    };
    pitcherCache.set(key, { value, expiresAt: Date.now() + PITCHER_CACHE_TTL });
    return value;
  } catch {
    pitcherCache.set(key, { value: empty, expiresAt: Date.now() + 15 * 60 * 1000 });
    return empty;
  }
}
...
async function getPitcher(competitor: EspnCompetitor): Promise<NrfiPitcher> {
  ...
  let era = parsePitcherStat(probable, "ERA");
  ...
  if (name && (era === null || whip === null)) {
    ...
    if (whip === null) whip = fallback.whip;
    if (era !== null || whip !== null) source = "MLB";
  }
  ...
  return {
    ...
    source: era !== null || whip !== null ? source : "pending",
  };
}
...
): Promise<Pick<NrfiGame, "nrfiProbability" | "recommendation" | "playStatus" | "modelEdge" | "confidence" | "sampleSize" | "factors">> {
