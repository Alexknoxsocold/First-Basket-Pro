...
type EspnSummary = {
  plays?: { period?: { number?: number }; inning?: number; homeScore?: number; awayScore?: number }[];
};

type TeamForm = { games: number; noRunPct: number; firstInningRuns: number; firstInningAllowed: number };
type CachedSchedule = { events: EspnEvent[]; expiresAt: number };
...
function getPitcher(competitor: EspnCompetitor): NrfiPitcher {
  ...
  return { name: probable?.athlete?.displayName ?? probable?.athlete?.fullName ?? null, era: Number.isFinite(era) ? era : null, headshot: probable?.athlete?.headshot ?? null };
}
...
async function fetchFirstInningFromSummary(eventId: string): Promise<{ away: number; home: number } | null> {
  ...
  try {
    ...
    const inningOne = plays.filter(play => (play.inning ?? play.period?.number) === 1);
    if (inningOne.length === 0) { gameFirstInningCache.set(eventId, null); return null; }
    let away = 0, home = 0;
...
async function fetchTeamForm(teamId: string, beforeDate: string): Promise<TeamForm> {
  ...
  try {
    ...
    const completed = events.filter(event => (event.date ?? "").slice(0, 10) < beforeDate)
      .filter(event => { const state = event.competitions?.[0]?.status?.type?.state; return state === "post" || event.competitions?.[0]?.status?.type?.completed === true; })
    ...
    const history = await withConcurrency(completed, async event => {
      ...
      return { teamRuns: teamIsAway ? score.away : score.home, opponentRuns: teamIsAway ? score.home : score.away };
    }, 6);
    const valid = history.filter((x): x is { teamRuns: number; opponentRuns: number } => x !== null);
    if (!valid.length) { teamFormCache.set(cacheKey, { value: fallback, expiresAt: Date.now() + 5 * 60 * 1000 }); return fallback; }
    const priorWeight = 3;
