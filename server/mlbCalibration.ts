...
function isGraded(row: CalibrationRow): boolean { return (row.recommendation === "NRFI" || row.recommendation === "YRFI") && (row.outcome === "NRFI" || row.outcome === "YRFI"); }
...
function unitProfit(row: CalibrationRow): number | null {
  if (!row.valuePlay || !row.marketAvailable || row.marketOdds === null || !isGraded(row)) return null;
  const decimal = decimalOdds(row.marketOdds); if (decimal === null) return null;
  return didWin(row) ? decimal - 1 : -1;
}
function bucketFor(probability: number): { label: string; min: number; max: number } {
  const p = Math.min(0.95, Math.max(0.50, probability)); const lower = Math.floor((p * 100) / 5) * 5;
...
function calculatePerformance(rows: CalibrationRow[]): PerformanceSummary {
  const bets = rows.filter(row => unitProfit(row) !== null);
  const wins = bets.filter(didWin).length; const losses = bets.length - wins;
  ...
  return {
    bets: bets.length, wins, losses, pushes: 0, winRate: bets.length ? wins / bets.length : null,
    unitsRisked: bets.length, unitsProfit: Math.round(unitsProfit * 10000) / 10000,
    roi: bets.length ? unitsProfit / bets.length : null,
    averageOdds: odds.length ? odds.reduce((sum, v) => sum + v, 0) / odds.length : null,
    averageEdge: edges.length ? edges.reduce((sum, v) => sum + v, 0) / edges.length : null,
    averageExpectedValue: evs.length ? evs.reduce((sum, v) => sum + v, 0) / evs.length : null,
  };
}

function calculateSummary(rows: CalibrationRow[]): CalibrationSummary {
  const graded = rows.filter(isGraded); const wins = graded.filter(didWin).length;
  const averagePredicted = graded.length ? graded.reduce((sum, row) => sum + safeProbability(row.probability), 0) / graded.length : null;
  const buckets: CalibrationBucket[] = [];
  for (let lower = 0.50; lower < 0.95; lower += 0.05) {
    const upper = lower + 0.05;
    const inBucket = graded.filter(row => { const p = safeProbability(row.probability); return p >= lower && (p < upper || (upper >= 0.95 && p <= upper)); });
    const bucketWins = inBucket.filter(didWin).length;
    const avg = inBucket.length ? inBucket.reduce((sum, row) => sum + safeProbability(row.probability), 0) / inBucket.length : null;
    const hitRate = inBucket.length ? bucketWins / inBucket.length : null;
    buckets.push({ label: `${Math.round(lower * 100)}-${Math.round(upper * 100)}%`, minProbability: lower, maxProbability: upper, predictions: inBucket.length, wins: bucketWins, hitRate, averagePredicted: avg, calibrationGap: hitRate !== null && avg !== null ? hitRate - avg : null });
  }
  ...
  const predictionDates = rows.map(row => row.lockedAt ?? row.createdAt).filter(Boolean).map(value => new Date(value as Date | string).getTime()).filter(Number.isFinite);
  const gradedDates = graded.map(row => row.gradedAt).filter(Boolean).map(value => new Date(value as Date | string).getTime()).filter(Number.isFinite);
  return {
    sampleSize: rows.length, gradedPredictions: graded.length, wins, hitRate: graded.length ? wins / graded.length : null,
    brierScore, logLoss, expectedCalibrationError: ece, averagePredicted, buckets,
    qualifiedPlays: { sampleSize: qualified.length, wins: qualifiedWins, hitRate: qualified.length ? qualifiedWins / qualified.length : null, averagePredicted: qualified.length ? qualified.reduce((sum, row) => sum + safeProbability(row.probability), 0) / qualified.length : null },
    performance: calculatePerformance(rows),
...
export async function getCalibrationSummary(days = 30): Promise<CalibrationSummary> {
  const db = getPool(); if (!db) return calculateSummary([]); await ensureTable();
  const safeDays = Math.min(Math.max(Math.round(days), 1), 365);
  const result = await db.query<CalibrationRow>(
    `SELECT prediction_date AS "predictionDate", game_id AS "gameId", recommendation, probability, outcome,
            value_play AS "valuePlay", market_available AS "marketAvailable", market_odds AS "marketOdds",
            market_edge AS "marketEdge", market_expected_value AS "marketExpectedValue", locked_at AS "lockedAt",
      ...
      ORDER BY prediction_date DESC, locked_at DESC NULLS LAST, created_at DESC`, [safeDays]);
...
export async function calibrateRecommendedProbability(rawProbability: number): Promise<number> {
  const db = getPool(); if (!db || !Number.isFinite(rawProbability)) return rawProbability; await ensureTable();
  const raw = Math.min(0.999, Math.max(0.001, rawProbability)); if (raw < 0.50 || raw > 0.95) return raw;
  const total = await db.query<{ count: string }>(`SELECT COUNT(*)::text AS count FROM mlb_prediction_snapshots WHERE recommendation IN ('NRFI','YRFI') AND outcome IN ('NRFI','YRFI')`);
  if (Number(total.rows[0]?.count ?? 0) < 50) return raw;
  const bucket = bucketFor(raw);
  const result = await db.query<{ n: string; wins: string }>(`SELECT COUNT(*)::text AS n, COUNT(*) FILTER (WHERE outcome = recommendation)::text AS wins FROM mlb_prediction_snapshots WHERE recommendation IN ('NRFI','YRFI') AND outcome IN ('NRFI','YRFI') AND probability >= $1 AND probability < $2`, [bucket.min, bucket.max]);
  const n = Number(result.rows[0]?.n ?? 0); if (n < 10) return raw;
  const wins = Number(result.rows[0]?.wins ?? 0); const empirical = wins / n; const calibrated = (wins + raw * 20) / (n + 20);
  const bounded = Math.min(Math.max(raw, empirical), Math.max(raw, calibrated));
  return Math.round(bounded * 1000) / 1000;
}

export async function backfillWalkForward(days: number, fetchForDate: (date: string) => Promise<{ date: string; games: Array<any> }>): Promise<{ datesProcessed: number; predictionsWritten: number; gamesGraded: number }> {
  ...
  let predictionsWritten = 0; let gamesGraded = 0;
  for (const date of dates) { const response = await fetchForDate(date); gamesGraded += response.games.filter((game: any) => game.outcome === "NRFI" || game.outcome === "YRFI").length; }
