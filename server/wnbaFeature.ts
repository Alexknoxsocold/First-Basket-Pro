...
export function registerWnbaFeature(app:Express):void{
  ...
  void Promise.all([runWnbaTracker(),backfillWnbaHistory(5,16),refreshRecentWnbaEvidence(10)]).then(([t,r,v])=>console.log(`[WNBA] Startup strict tracker ${t.processed} processed/${t.unresolved} unresolved; rebuild added ${r.gamesAdded}; evidence ${v.updated}/${v.checked}.`)).catch(e=>console.warn('[WNBA] Startup strict maintenance failed:',e));
  console.log('[WNBA] First Basket initialized: projections, locks, strict grading and strict history rebuild active.');
}
