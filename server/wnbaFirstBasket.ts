...
async function playerTipStats(name:string,team:string,season:number){if(!pool)return{wins:0,events:0,pct:null as number|null};await ensureWnbaSchema();const r=await pool.query(`SELECT tip_winner_team,tip_player_a,tip_player_b,team_a,team_b FROM wnba_opening_evidence WHERE extract(year from game_date)::int IN ($1::int, ($1::int - 1)) AND confidence='verified' AND tip_winner_team IS NOT NULL AND (upper(team_a)=upper($2) OR upper(team_b)=upper($2))`,[season,team]);const rows=r.rows.filter(x=>sameTipPlayer(String(x.tip_player_a||''),name)||sameTipPlayer(String(x.tip_player_b||''),name));const events=rows.length,wins=rows.filter(x=>normalizeTeam(String(x.tip_winner_team||''))===normalizeTeam(team)).length;return{wins,events,pct:events?Math.round(wins/events*1000)/10:null}}
