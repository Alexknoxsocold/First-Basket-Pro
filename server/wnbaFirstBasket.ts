...
function propSignalKey(v:string){return v.toLowerCase().normalize('NFKD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]/g,'')}
...
async function latestProjectedStarters(team:string):Promise<Starter[]>{
  ...
  const ranked=await Promise.all(availableRoster.map(async(player:any)=>{
    ...
    const stats=player?.id?await fetchPlayerStats(String(player.id),season):null;
    ...
    const propBoost=propSignal?Math.min(22,(propSignal.marketCount*3)+(Math.min(propSignal.bookCount,4)*1.5)):0;
    const score=(startRate*90)+(recentStarts*8)+(starts*0.5)+(minutes*1.2)+recencyBonus+propBoost+availabilityAdjustment(player);
    return {norm,name,score,startRate,recentStarts,starts,minutes,propMarkets:propSignal?.marketCount||0,propBooks:propSignal?.bookCount||0};
  }));
  ...
  console.log('[WNBA Projection]',key,ranked.slice(0,8).map(x=>({name:x.name,score:Math.round(x.score*10)/10,startRate:Math.round(x.startRate*100),recentStarts:x.recentStarts,propMarkets:x.propMarkets,propBooks:x.propBooks})));
