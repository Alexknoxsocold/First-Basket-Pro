...
export async function getMlbIntegritySummary(days = 30): Promise<MlbIntegritySummary> {
  ...
  const issueQueries: Array<{ severity: "ERROR" | "WARNING"; code: string; description: string; sql: string }> = [
    ...
    { severity: "ERROR", code: "MARKET_BEFORE_LOCK", description: "A captured market timestamp is later than the lock, which is not a valid pre-lock price.", sql: `SELECT COUNT(*)::text AS count FROM mlb_prediction_snapshots WHERE prediction_date >= to_char(current_date - ($1::int - 1), 'YYYY-MM-DD') AND locked_at IS NOT NULL AND market_captured_at IS NOT NULL AND market_captured_at > locked_at` },
  ...
  return {
    generatedAt: new Date().toISOString(), windowDays: safeDays,
    snapshots: Number(counts?.snapshots ?? 0), locked: Number(counts?.locked ?? 0), graded: Number(counts?.graded ?? 0), marketCaptured: Number(counts?.marketCaptured ?? 0),
    errors, warnings, status, issues,
  };
}
