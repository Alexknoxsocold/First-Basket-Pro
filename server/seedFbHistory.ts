/**
 * Seeds verified 2025-26 first-basket history.
 * These values are historical priors and must never overwrite another season.
 */

import { upsertFirstBasketPlayerSeason } from './fbSeasonStore';

const SEED_SEASON = '2025/26';
...
export async function seedFbHistoryFromBestOdds(): Promise<void> {
  if (seeded) return;
  seeded = true;
  console.log(`[FBSeed] Seeding verified ${SEED_SEASON} first-basket history...`);
  let count = 0;
  for (const entry of BESTODDS_DATA) {
    try {
      await upsertFirstBasketPlayerSeason(entry.playerName, entry.team, entry.fbScored, entry.gamesTracked, SEED_SEASON);
      count++;
    } catch (err) {
      console.warn(`[FBSeed] Failed for ${entry.playerName}:`, err);
    }
  }
  console.log(`[FBSeed] Done — seeded ${count}/${BESTODDS_DATA.length} players into ${SEED_SEASON}.`);
}
