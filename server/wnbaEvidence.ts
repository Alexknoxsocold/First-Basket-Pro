...
function parseJump(p:any,starters:WnbaStarter[]){
  const text=String(p?.text||''); if(!/jump ball/i.test(text))return null;
  const participantNames=(p?.participants||[]).map((x:any)=>String(x?.athlete?.displayName||'').trim()).filter(Boolean);
  const vs=text.match(/jump ball\s+(.+?)\s+vs\.?\s+(.+?)(?:\s*\(|$)/i);
  const a=participantNames[0]||vs?.[1]?.trim()||null,b=participantNames[1]||vs?.[2]?.trim()||null;
  const teams=[...new Set(starters.map(s=>normTeam(s.team)))];
  let winnerTeam=playTeam(p); if(!teams.includes(winnerTeam))winnerTeam='';
  if(!winnerTeam){
    const possession=text.match(/\((.+?)\s+(?:gains|controls|gets)\s+possession\)/i)||text.match(/(?:controlled by|possession to)\s+(.+?)(?:\.|$)/i);
    const possName=possession?.[1]?.trim()||''; const possStarter=possName?starterFor(possName,'',starters):null;
    if(possStarter)winnerTeam=normTeam(possStarter.team);
  }
  if(!winnerTeam){
    for(const part of p?.participants||[]){const t=normTeam(String(part?.team?.abbreviation||part?.athlete?.team?.abbreviation||''));const typ=String(part?.type||'').toLowerCase();if(t&&teams.includes(t)&&/(winner|possession|controlled)/.test(typ)){winnerTeam=t;break}}
  }
  return{winnerTeam:winnerTeam||null,a,b};
}
...
export function verifyOpeningEvidence(gameId:string,gameDate:string,summary:any,starters:WnbaStarter[]):WnbaOpeningEvidence|null{
  ...
  const jump=plays.map(p=>parseJump(p,starters)).find(Boolean) as {winnerTeam:string|null;a:string|null;b:string|null}|undefined;
