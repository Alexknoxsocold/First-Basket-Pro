...
**Authentication System**: Custom email/password authentication with bcrypt password hashing and persistent PostgreSQL session storage via express-session and connect-pg-simple. Authentication is available but optional - all pages are publicly accessible without login. Sessions expire after 30 days and persist across server restarts. Cookie security adapts to deployment environment (httpOnly, sameSite: 'lax', secure flag based on NODE_ENV). AuthContext provides login/signup/logout functions and manages session state with AbortController to prevent memory leaks.
...
**ESPN Player Stats Service**: `server/espnPlayerStats.ts` fetches real 2025-26 season stats for ALL active players on today's game teams (not just starters). The new `fetchEspnTeamStats` function (1) fetches full rosters for all 6 teams playing today in parallel, (2) filters out players who are definitively OUT, (3) fetches season stats for all active players (PPG, FGA/g, FG%, assists, rebounds, minutes), then (4) derives first basket probability using FGA rate, scoring efficiency, minutes played, and position. Results include player headshots, injury badges, and American odds. **Server-side cache**: Results are cached for 5 minutes to enable instant page loads. Cache is invalidated on ESPN lineup sync. Endpoint: `GET /api/espn-player-stats`. Coverage: ~79 players across 6 teams for today's games.
**Player FB Stats Page**: Redesigned as a BettingPros-style first basket model. Shows all 79 active players sorted by First Basket %. Features: Top 5 picks banner, sortable table (FB%, Odds, PPG, MIN, FGA/G, FG%, Q1 FGA, GP), green highlighting for above-average stats, game filter tabs, Starters Only toggle, and per-player matchup context (opponent, game time). Color scale: green=elite 28%+, yellow=good 20-27%, grey=low <20%.
**Data Models**: Uses Drizzle ORM for PostgreSQL with express-session for persistent session storage. Application data (games, players, teams) currently stored in-memory via MemStorage, while sessions are persisted to PostgreSQL. Zod for validation. Schemas include Users (id, email, passwordHash, role, createdAt), Games (matchup info, jump ball, tips, start times, starting lineups), Player Stats (player ID, team, position, games played, first basket occurrences, Q1 FGA Rate, Last 10 FB%, injury status, betting odds), and Team Stats (team ID, games played, first-to-score occurrences). Session table managed automatically by connect-pg-simple.
