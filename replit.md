...
**Authentication System**: Custom email/password authentication with bcrypt password hashing and persistent PostgreSQL session storage via express-session and connect-pg-simple. All pages require authentication - users must sign up or log in to access any content. Sessions expire after 30 days and persist across server restarts. Cookie security adapts to deployment environment (httpOnly, sameSite: 'lax', secure flag based on NODE_ENV). AuthContext provides login/signup/logout functions and manages session state with AbortController to prevent memory leaks. ProtectedRoute component wraps all pages except /signup and /login, redirecting unauthenticated users to login page (default).
