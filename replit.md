...
**ESPN Player Stats Service**: `server/espnPlayerStats.ts` fetches real 2025-26 season stats for today's starters from ESPN's core API (`sports.core.api.espn.com/v2/...`). For each starter, it (1) fetches their team's current ESPN roster to find the player's ESPN ID (with a cross-team global fallback to handle traded players), (2) fetches season stats (PPG, FGA/g, FG%, assists, rebounds, minutes), then (3) derives first basket probability using FGA rate, scoring efficiency, minutes played, and position. Results include player headshots, injury badges, and American odds. Endpoint: `GET /api/espn-player-stats`. Coverage: ~85% of starters (remaining are traded to non-playing teams or have no 2025-26 stats).
